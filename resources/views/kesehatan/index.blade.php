                            ...
                            <div class="table-responsive">
                                <table class="table table-hover" id="table_{{ $penyakit->nama_penyakit }}">
                                    ...
                                    <tr>
                                        ...
                                        <th>Alamat</th>
                                        <th>Tgl Terdampak</th>
                                        @if(Auth::user()->level == 'admin' ||Auth::user()->level == 'RT' && Auth::check())
                                        <th>Aksi</th>
                                        @endif
                                    </tr>
                                    </thead>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="NIK_penduduk" id="NIK_penduduk" class="form-select">
                                ...
                                @endforeach
                                    <label for="searchPenduduk" class="form-label">Cari Penduduk:</label>
                                    <input type="text" class="form-control" id="searchPenduduk" placeholder="Masukkan nama penduduk">
                            </select>
                        </div>
                        <div class="mb-3">
...
@section('scripts')
    <script>
        // Fungsi untuk melakukan pencarian secara langsung saat pengguna mengetik
        function searchPenduduk() {
            var input, filter, select, option, i;
            input = document.getElementById('searchPenduduk');
            filter = input.value.toUpperCase();
            select = document.getElementById("NIK_penduduk");
        ...
        // Panggil fungsi searchPenduduk() saat pengguna mengetik
        document.getElementById('searchPenduduk').addEventListener('input', function() {
            searchPenduduk();
        });

        // Fungsi untuk menangani pemilihan penduduk dari hasil pencarian
        function selectPenduduk(selectedPenduduk) {
            var pendudukId = selectedPenduduk.value;
            var pendudukName = selectedPenduduk.text;
            $('#NIK_penduduk').val(pendudukId);
        }

        // Panggil fungsi selectPenduduk() saat pengguna memilih dari hasil pencarian
        document.getElementById('NIK_penduduk').addEventListener('change', function() {
            var selectedPenduduk = this.options[this.selectedIndex];
            selectPenduduk(selectedPenduduk);
        });
    </script>
@endsection
